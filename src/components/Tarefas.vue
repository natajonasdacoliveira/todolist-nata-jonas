  ...
  <q-page class="flex">
    <q-list class="full-width" bordered separator padding>
      ...
      <q-item v-for="(tarefa, index) in tarefas" :key="index">
        <q-expansion-item
          ref="expansionItem"
          @click="setDropdown(index, tarefa);"
          class="full-width"
          switch-toggle-side
          group="listaTarefas"
        >
          <template v-slot:header>
            ...
            <q-item-section side>
              <template v-if="tarefa.status_tarefa">
                <q-checkbox
                  class="flex-center"
                  @input="finalizarTarefa(tarefa.id_tarefa)"
                  v-model="tarefa.status_tarefa"
                  color="green"
                />
              </template>
...
export default {
  ...
  methods: {
    ...
    finalizarTarefa(id_tarefa) {
      if (this.estadoDropdown == true) {
        this.$store.dispatch("setEstadoDropdown", false);
        this.$store.dispatch("setIndexDropdown", -1);
        let length = this.$refs.expansionItem.length;
        for (let index = 0; index < length; index++) {
          this.$refs.expansionItem[index].hide();
        }
      }
      this.$store.dispatch("finalizarTarefa", id_tarefa);
    }
  },
